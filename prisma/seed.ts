...
async function main() {
  ...
  // ─── SellerProduct listings (multiple sellers per product) ────────────────────
  // seller1 lists all their own products
  await Promise.all(
  ...
  // seller2 lists some products at slightly different prices
  await prisma.sellerProduct.create({
    data: {
      productId: products[0]!.id, // Organic Tomatoes
      sellerId: seller2.id,
      price: 3.2,
      stock: 75,
    },
  ...
  await prisma.sellerProduct.create({
    data: {
      productId: products[1]!.id, // Fresh Carrots
      sellerId: seller2.id,
      price: 1.8,
      stock: 90,
    },
  ...
  await prisma.sellerProduct.create({
    data: {
      productId: products[3]!.id, // Organic Apples
      sellerId: seller2.id,
      price: 3.8,
      stock: 40,
    },
  ...
  await prisma.sellerProduct.create({
    data: {
      productId: products[2]!.id, // Green Lettuce
      sellerId: seller3.id,
      price: 1.4,
      stock: 30,
    },
  ...
  // seller4 lists some products at premium prices
  await prisma.sellerProduct.create({
    data: {
      productId: products[0]!.id, // Organic Tomatoes
      sellerId: seller4.id,
