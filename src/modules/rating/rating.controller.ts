...
export const submitRating = async (req: AuthRequest, res: Response) => {
  try {
    const { orderId, driverId, sellerId, buyerId, ratings, comment } = req.body;
    if (!orderId || !driverId || !sellerId || !buyerId || !ratings) {
      return res.status(400).json({ message: 'Missing required fields' });
    }
    const rating = await createRating({ orderId, driverId, sellerId, buyerId, ratings, comment });
...
// GET /api/v1/rating/seller/:sellerId  ← new
...
// GET /api/v1/rating/seller/:sellerId/stats  ← new
...
// GET /api/v1/rating/my-seller-ratings  ← new (seller calls this, gets their own reviews)
