...
export const createRating = async (input: RatingInput) => {
  ...
  await createNotification({
    ...
    body:   `${buyerName} rated your delivery ${stars} (${input.ratings.delivery}/5)`,
    data:   { type: 'rating', orderId: input.orderId },
  }).catch(() => {});

  // ---- notify the seller ----
  const sellerUserId = rating.seller.user.id;
  await createNotification({
    userId: sellerUserId,
    title:  'You received a new review! ⭐',
    body:   `${buyerName} rated your store ${stars} (${input.ratings.overall}/5)${input.comment ? ` — "${input.comment}"` : ''}`,
    data:   { type: 'rating', orderId: input.orderId },
  }).catch(() => {});
